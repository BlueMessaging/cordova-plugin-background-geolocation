...
public class LocationService extends Service {
    ...
    /**
     * Command sent by the service to any registered clients with error.
     */
    public static final int MSG_ERROR = 1;

    /**
     * Command to the service to register a client, receiving callbacks from the
     * service. The Message's replyTo field must be a Messenger of the client where
    ...
    /**
     * Command sent by the service to any registered clients with the new position.
     */
    public static final int MSG_LOCATION_UPDATE = 4;

    /**
     * Command sent by the service to any registered clients whenever the devices
    ...
    /**
     * Command sent by the service to any registered clients with new detected
     * activity.
     */
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        if (mConfig.getStartForeground()) {
            Notification notification = new NotificationFactory().getNotification(mConfig.getNotificationTitle(),
        ...
        public Notification getNotification(String title, String text, String largeIcon, String smallIcon,
            ...
            if (largeIcon != null && !largeIcon.isEmpty()) {
                builder.setLargeIcon(
                        BitmapFactory.decodeResource(getApplication().getResources(), getDrawableResource(largeIcon)));
            }
     ...
     *
     * All locations updates are recorded in local db at all times. Also location is
     * also send to all messenger clients.
     *
     * If option.url is defined, each location is also immediately posted. If post
     * is successful, the location is deleted from local db. All failed to post
     * locations are coalesced and send in some time later in one single batch.
     * Batch sync takes place only when number of failed to post locations reaches
     * syncTreshold.
     *
